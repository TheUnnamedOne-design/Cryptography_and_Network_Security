...
public class SHA512 {
    ...
    private static final long[] K = {
        ...
        0x650a73548baf63deL, 0x766a0ebb3c88b691L, 0x81c2c92e47edaee6L, 0x92722c851482353bL,
        0xa2bfe8a14cf10364L, 0xa81a664bbc423001L, 0xc24b8b70d0f89791L, 0xc76c51a30654be30L,
        0xd192e819d6ef5218L, 0xd69906245565a910L, 0xf40e35855771202aL, 0x106aa07032bbd1b8L,
        0x19a4c116b8d2d0c8L, 0x1e376c08d5d0d465L, 0x2748774cdf8eeb99L, 0x34b0bcb5e19b48a8L,
    ...
    private long rotr(long n, int d) {
        return (n >>> d) | (n << (64 - d));
    }
    ...
    private byte[] padMessage(byte[] message) {
        long lInBytes = message.length;
        long blInBits = lInBytes * 8;

        int paddingLength = (int) ((lInBytes % 128 < 112) ? (112 - lInBytes % 128) : (240 - lInBytes % 128));

        byte[] padded = new byte[(int) lInBytes + paddingLength + 16];
        System.arraycopy(message, 0, padded, 0, (int) lInBytes);

        padded[(int) lInBytes] = (byte) 0x80;

        // SHA-512 uses 128-bit block for length (in big-endian).
        // For short messages, the first 8 bytes of the length block are 0.
        for (int i = 0; i < 8; i++) {
            padded[padded.length - 1 - i] = (byte) (blInBits >>> (8 * i));
        }
    ...
    public String hash(String m) {
        ...
        int numBlocks = padded.length / 128;

        for (int q = 0; q < numBlocks; q++) {
            long[] w = new long[80];
            int blockStart = q * 128;

            for (int j = 0; j < 16; j++) {
                int index = blockStart + j * 8;
                w[j] = ((long) (padded[index] & 0xff) << 56)
        ...
        return String.format("%016x%016x%016x%016x%016x%016x%016x%016x", h0, h1, h2, h3, h4, h5, h6, h7);
    }
}
